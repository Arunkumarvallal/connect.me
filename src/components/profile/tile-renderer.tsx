...
export function TileRenderer({ tile, isDashboard, onRemove, onEdit }: TileRendererProps) {
  ...
  const commonClasses = cn(
    "relative overflow-hidden group transition-all duration-500 hover:shadow-2xl rounded-[2.5rem] border border-black/5",
    sizeClass
  );
  ...
  const EditOverlay = () => isDashboard && (
    <div className="absolute inset-0 bg-black/40 opacity-0 group-hover:opacity-100 transition-opacity flex items-center justify-center gap-2 z-10 backdrop-blur-sm">
      <Button size="icon" variant="secondary" className="rounded-full" onClick={() => onEdit?.(tile)}>
        <Edit2 size={16} />
      </Button>
      <Button size="icon" variant="destructive" className="rounded-full" onClick={() => onRemove?.(tile.id)}>
        <Trash2 size={16} />
      </Button>
    </div>
  );
  ...
  switch (tile.type) {
    case 'social':
      ...
      return (
        <Card className={cn(commonClasses, isLinkedIn ? "bg-white" : "bg-muted/30", "flex flex-col items-center justify-center p-6")}>
          ...
          {tile.size !== '1x1' && <span className="mt-2 text-sm font-bold opacity-60 tracking-tight">{tile.metadata?.brand}</span>}
    ...
    case 'image':
      return (
        <Card className={cn(commonClasses, "p-0 min-h-[140px]")}>
          <EditOverlay />
            ...
            <div className="relative w-full h-full">
              ...
              {(tile.title || tile.content) && (
                <div className="absolute inset-0 bg-gradient-to-t from-black/60 to-transparent flex flex-col justify-end p-8 text-white">
                  <h4 className="text-2xl font-black uppercase tracking-tighter">{tile.title}</h4>
                  <p className="text-sm opacity-80">{tile.content}</p>
                </div>
              )}
            </div>
          )}
    ...
    default:
      return (
        <Card className={cn(commonClasses, "bg-white p-6")}>
          <EditOverlay />
          <h4 className="font-bold">{tile.title || 'New Tile'}</h4>
          <p className="text-sm opacity-60">{tile.content}</p>
        </Card>
      );
  }
}
